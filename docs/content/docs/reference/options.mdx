...
export const auth = betterAuth({
	emailAndPassword: {
		...
		onPasswordReset: async ({ user }) => {
			// Do something when the password is reset
		},
...
- `onPasswordReset`: A callback function that is triggered when a user's password is changed successfully.
...
- `revokeSessionsOnPasswordReset`: Whether to revoke all other sessions when resetting password (default: `false`)
